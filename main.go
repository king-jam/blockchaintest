...
func calculateHash(block Block) string {
	...
	if err := binary.Write(record, binary.LittleEndian, block.Index); err != nil {
		return ""
	}
	if err := binary.Write(record, binary.LittleEndian, block.Timestamp.UnixNano()); err != nil {
		return ""
	}
	_, err := record.Write(block.Data)
	if err != nil {
		return ""
	}
	_, err = record.Write([]byte(block.PrevHash))
	h := sha256.New()
